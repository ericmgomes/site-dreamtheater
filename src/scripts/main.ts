...
document.querySelectorAll<HTMLButtonElement>('[data-copy-anchor]').forEach(button => {
  const copyStatus = button.parentElement?.querySelector<HTMLElement>('[data-copy-status]');
  let resetStatus: ReturnType<typeof setTimeout>;
  button.hidden = false;
  button.addEventListener('click', async () => {
    ...
    try {
      ...
      if (copyStatus) copyStatus.textContent = 'Link copiado!';
      clearTimeout(resetStatus);
      resetStatus = setTimeout(() => { if (copyStatus) copyStatus.textContent = ''; }, 3000);
    } catch {
